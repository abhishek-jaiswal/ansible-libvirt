...
# (c) 2015, Abhishek Jaiswal <ia.malhotra@gmail.com>
#
...
DOCUMENTATION = '''
---
author: "Abhishek Jaiswal (@abhishek_jais)"
module: virtinstall
...
def main():
    module = AnsibleModule(
        argument_spec = dict(
            ...
            ram=dict(type='str', required=True),
            vcpus=dict(type='str', required=True),
	...
	for key, value in param_dict.items():
	    string += "--network " + "bridge=" + value['bridge'].strip() + ",portgroup=" + value['portgroup'].strip() + ",model=virtio " 
	return string
    ...
    if module.check_mode:
        ...
        import_cmd = "%s --name=%s --ram=%s --vcpus=%s,cores=%s --virt-type=%s --os-type=%s %s --noautoconsole %s --import" %(virt_install, name, ram, vcpus, vcpus, virt_type, os_type, disk, dict_to_param(network))
        if pxe and force:
